...
public:
    int minimumEffortPath(vector<vector<int>>& heights) {
        ...
        while (!minHeap.empty()) {
            ...
            if (currI != heights.size()-1) {
                int temp = max(currEffort, abs(heights[currI+1][currJ]-heights[currI][currJ]));
                if (temp < knownEffort[currI+1][currJ]) minHeap.push({temp, currI+1, currJ});
            }
            if (currI != 0) {
                int temp = max(currEffort, abs(heights[currI-1][currJ]-heights[currI][currJ]));
                if (temp < knownEffort[currI-1][currJ]) minHeap.push({temp, currI-1, currJ});
            }
            if (currJ != heights[0].size()-1) {
                int temp = max(currEffort, abs(heights[currI][currJ+1]-heights[currI][currJ]));
                if (temp < knownEffort[currI][currJ+1]) minHeap.push({temp, currI, currJ+1});
            }
            if (currJ != 0) {
                int temp = max(currEffort, abs(heights[currI][currJ-1]-heights[currI][currJ]));
                if (temp < knownEffort[currI][currJ-1]) minHeap.push({temp, currI, currJ-1});
